...
<script>
    generateQuiz(
        ...
        {
        'Exhaustive Grid Search (<code>GridSearchCV</code>)': 'Randomized grid search picks the best result more often than not and we get control over the amount of interations that occur.',
        'Randomized Grid Search (<code>RandomizedSearchCV</code>)': '',
        'Both are relatively fast': 'Randomize grid search can be much faster than exhaustive grid search since we can pick the amount of interations that occur.',
        },
        'Randomized Grid Search (<code>RandomizedSearchCV</code>)',
    );
</script>
