...
We're going to think about 3 types of scores:<br><br>
- **Training score**: The score that our model gets on the same data that it was trained on. (seen data - training data)<br>
- **Validation score**: The mean validation score from cross-validation.<br>
- **Test score**: This is the score from the data that we locked away.<br>
:::
...
y_encoded = np.where(y == 'Canada', 1, 0)
...
If the scores are very far apart then we are more likely **overfitting**.<br><br>    
- In this case, the solution would be to reduce the complexity of the model.<br>

If the scores are very close together then we are more likely **underfitting**.<br><br>
