def test():
    ...
    assert precision == 0.92, "The value for precision is incorrect. Are you scoring properly?"
    assert "recall_score" in __solution__, "Make sure you are doing recall scoring on the validation data and the predicted data."
    assert recall == 0.742, "The value for recall is incorrect. Are you scoring properly?"
    assert "f1_score" in __solution__, "Make sure you are doing f1 scoring on the validation data and the predicted data."
    assert f1 == 0.821, "The value for f1 is incorrect. Are you scoring properly?"
    assert "classification_report" in __solution__, "Make sure you are printing a classification report."
