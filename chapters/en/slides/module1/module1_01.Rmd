...
The first one being the ability to predict whether a patient has a liver disease or not.
...
Notes: First we obtain our data on our patients and wrangle it as necessary. 
...
Notes: Next, we building a model and train our model using the labels we already have. Ignore this output here. It's just explaining what's going on in the model which we will explain soon. 
...
Here we can see that our model is predicting an outcome of the patience under the `Predicted label` column. 
...
Notes: Here we already have a trained model which has been show thousands of images. If we give it images from our own collection, the model attempts to make a prediction of the contents of the image. In this case the model predicts the animal to be an `ox` with 86% probability. That's not bad. 
...
Notes: In this case it's not so good it's confident in identifying the animal. The model in this case thinks our donkey image, is a llama but only with a probability of `0.12`. 
...
Notes: We also also use machine learning to predict negative or positive sentiment expressed in a movie review.
...
Notes:  First we wrangle our data so that we can train our model. This data contains the review as a column and a `label` column which contains values of either `pos` or `neg` for positive or negative. 
...
Notes: Next, we build our model and train on our existing data. Don't worry about the code here. 
...
Notes: Once we have our model trained, we can then  predict on data we haven't seen before using the model we just built. 
...
Notes: We wrangle our data just as we did before. These data consist of the characteristics of houses in King County, USA. 
