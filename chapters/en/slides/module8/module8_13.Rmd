...
```{python  echo=FALSE, fig.width = 13, fig.height = 8,  out.width = '80%', fig.align='center'}
...
But this won't work with probabilities since we get values that are negative or greater than 1. 

We use something called a **sigmoid function**  "squashes" the raw model output from any number to the range  [0,1].
...
We can see that the first example was incorrectly predicted as "Canada" instead of "USA" but we also see that the model what not extremely confident in this prediction. It was 69.8% confident. 

The rest of this selection the model corrected predicted but the model was more confident in some than others. 
...
When we use predict we get a decision boundary with either blue or red, a colour for each class but now, with probabilities, we can see that the model is less confident the closer the observations are to the decision boundary. 
...
Let's find an example that the model is pretty confident. 
This time where we make our dataframe we are only bringing in the probability of predicting "Canada". This is because we are 0 percent confident a prediction is "Canada", the model is 100% confident in "USA".

Here we can see both extremes. 

Like we are 99.345% (1- 0.006547) confident that city 37 is "USA" and 96.19% confident that city 1 is "Canada". The firt one it got right, but the second one it, didn't, let's plot them and see why. 
...
Both points are "USA" cities but the second one we can now see why the model was more confident in this one. This city is likely in Alaska but the model doesn't know that and predicts more so on how close it lies to the decision boundary. 
...
Let's now find a example where the model is less certain on it's prediction. 

We can do this by finding the absolute value of the difference between the two probabilities. The small the value, the more uncertain the model is. 
...
When we plot the cities with the decision boundary, we get a clear answer. The cities lies almost completely on the boundary making the model very divided between how to classify them 
