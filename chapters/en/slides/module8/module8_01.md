...
Let’s bring back our hypothetical snake data we saw in module 4 to
...
We can import LinearRegression in like all the previous models we’ve
used except we are going to instead focus on it’s close cousin `Ridge`.
...
`Ridge`, has the same fit-predict paradiagm as the other models we have
seen.

That means we can `fit` on training set and `predict` a numeric
prediction.

We see that running `predict` returns the predicted housing prices for
our examples.
...
The `alpha` hyperparameter is what make it more flexible than using
`LinearRegression`.

Remember the fundamental trade off we spoke about in module 3?

**“As model complexity ↑, Score\_train ↑ and Score\_train − Score\_valid
tend to ↑”**
...
In this plot, the green markers are the examples and our orange line is
our Ridge regression line.

If we had a additional feature, let’s say, `width`, we now would have 2
