...
It's very similar to the `Ridge` classifier we saw earlier but this one has some key differences. 
...
We also can see that the weight of latitude is larger in magnitude than that of longitude. 
...
Again, let's take an example from our test set and calculate the outcome using our weights and intercept. 
