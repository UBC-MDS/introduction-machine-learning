...
Probably, but let's keep them in for now. You could experiment by removing them and examine whether results change. 
...
It looks like the `total_bedrooms` and `bedrooms_per_household` columns have some missing values.
...
Looks like the data is missing 207 values for `total_bedrooms` and therefore the same amount for `bedrooms_per_household`.
