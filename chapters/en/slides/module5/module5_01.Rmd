...
Very often real-world datasets need to be transformed or ***preprocessed***
before we use them to build ML models.
...
Intuition: To predict whether a particular player is a point guard ('G') or a forward ('F') (query point)
...
Let's have a look at just 2 players and calculate the distance between them.
...
1. Create a feature transformer object. This is done in a similar way to how we create a model.
...
We can compare it with  `transformer` which is a transformer used to change the input representation like to scale numeric features.
...
The scores with scaled data are better compared to the unscaled data in the case of 𝑘-NNs.
