...
Now if we build our 𝑘-NN classifier we determine that it gets an even *worse* score of 0.50! 

Dummy classifiers are supposed to be a baseline and so why is it getting a better score than a model that is actually doing machine learning? What’s going on?
...
We can compare `predict` it with `transformer` which is a transformer used to change the input representation to scale numeric features.
