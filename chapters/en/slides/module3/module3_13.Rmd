...
```{python echo=FALSE, fig.width = 13, fig.height = 8,  out.width = '74%'}
model = DecisionTreeClassifier()
model.fit(X, y);
...
A standard overfitting scenario would be: **𝐸_train<𝐸_best<𝐸_valid**

If 𝐸_train is low, then we are in an overfitting scenario. It is fairly common to have at least a bit of this
 
𝐸_valid  cannot be smaller than  𝐸_best  basically by definition. In reality, we won't have them equal.
...
We can't see 𝐸_best but we can see 𝐸_train and 𝐸_test.
