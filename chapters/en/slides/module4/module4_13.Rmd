...
An intuitive way to do this is predict the grey point using the same label as the next "closest" point (𝑘 = 1)
...
We could also use the 3 closest points (𝑘 = 3) and let them **vote** on the correct class.
...
Here we have a single point we are calling `one_city`. It's the green triangle we see in the plot. 

----

<center><img src="/module4/point.png"  width = "70%" alt="404 image" /></center>
...
If we predict the closest point where 𝑘 = 1, we would predict a target of **Canada** (red) in this case.
